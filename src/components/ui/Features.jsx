...
function Features() {
  ...
  return (
   <div className="w-full  bg-white px-8 sm:px-12 md:px-24 lg:px-32 flex flex-col gap-10 pt-24 lg:pt-32">
      ...
      transition={{duration:1}}
      viewport={{ once: true }}
      className={`w-full flex items-center gap-6 justify-center flex-wrap`}>
         <button onClick={
      ...
      transition={{duration:1}}
      viewport={{ once: true }}
        className='px-5 md:px-8 py-3 rounded-lg font-medium font-[Outfit] text-black bg-gray-300 cursor-pointer mb-3 group hover:bg-purple-600 hover:text-white'  > <FontAwesomeIcon icon={faArrowDown} className='group-hover:translate-y-1 group-hover:text-white transition-all duration-100'/> View All Prompts</motion.button></Link>
      </div>
    </div>
  )
}
